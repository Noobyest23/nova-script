...
#define nova_std_decl(name) static std::shared_ptr<NovaValue> name(std::vector<std::shared_ptr<NovaValue>>& args)
...
NovaString* str = static_cast<NovaString*>(args[arg_i].get());
...
NovaInt* i = static_cast<NovaInt*>(args[arg_i].get());
...
NovaInt* nova_int = static_cast<NovaInt*>(args[arg_i].get());\
f = nova_int->CNum();\
}\
else {\
NovaFloat* nova_float = static_cast<NovaFloat*>(args[arg_i].get()); \
f = nova_float->CNum();\
}
...
NovaFloat* f = static_cast<NovaFloat*>(args[arg_i].get());
...
NovaBool* b = static_cast<NovaBool*>(args[arg_i].get());
...
NovaObject* o = static_cast<NovaObject*>(args[arg_i].get());
...
NovaArray* a = static_cast<NovaArray*>(args[arg_i].get());

#define funcget(f, arg_i) if (args[arg_i] and args[arg_i]->Type() != "Function") {\
PushError("Expected function as argument " + std::to_string(arg_i)); return nullptr;}\
NovaFunction* f = static_cast<NovaFunction*>(args[arg_i].get());

#define objbindmethod(objname, name) objname->PushBack(#name, std::make_shared<NovaFunction>(name))
